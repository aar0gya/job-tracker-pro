...
class JobTrackerApp {
  ...
  renderApplications(applications) {
    ...
    this.applicationsBody.innerHTML = applications
      .map((app) => {
        ...
        datasets: [
          {
            label: "Applications",
            data: [0, 0, 0, 0, 0],
            backgroundColor: [colors.applied, colors.interview, colors.offer, colors.rejected, colors.withdrawn],
            borderRadius: 8,
            barThickness: 40,
            // Custom property to store company names
            companies: [[], [], [], [], []],
          },
        ],
      },
          ...
          tooltip: {
            ...
            callbacks: {
              afterBody: (context) => {
                ...
                const companies = dataset.companies[dataIndex];
  ...
  async updateChart() {
    ...
    // Fetch all applications to get company names
    ...
    applications.forEach((app) => {
      if (stats.hasOwnProperty(app.status)) {
        stats[app.status]++;
        companies[app.status].push(app.company);
      }
    });

    const data = [stats.applied, stats.interview, stats.offer, stats.rejected, stats.withdrawn];
    const companyLists = [companies.applied, companies.interview, companies.offer, companies.rejected, companies.withdrawn];

    this.chart.data.datasets[0].data = data;
    this.chart.data.datasets[0].companies = companyLists;
    this.chart.update();
  }
}
